...
AS
BEGIN
	-- Exam degree must be multiples of 50
	IF @exam_degree % 50<> 0
	BEGIN
		-- Throw error if exam degree is not divisible by 50
		RAISERROR('Exam degree must be a multiple of 50',16,1)
		RETURN
	END
	
	DECLARE @maxExamDegree INT = (SELECT SUM(q.Degree) from Question q where q.Course_Id
	= @crs_id)
 ...
 -- Start generating exam questions
  ...
  BEGIN
    ...
    SET @questionId = (
      SELECT TOP 1 q.Id
      FROM Question q where q.Course_Id = @crs_id
      ORDER BY NEWID()
    )

    -- Insert a new row into the Questions_Consist_Exam table to associate the selected question with the exam
    IF NOT EXISTS (SELECT * from [dbo].[Question_Exam]      where [QuestionID]  = @questionId and
	ExamID = (@lastExamId+1))
	BEGIN
