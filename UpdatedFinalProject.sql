...
create table Question_Exam(
	...
	ExamID int not null,
	Degree int null
	constraint Qu_EX primary key(QuestionID,ExamID)
	)
go
--------------------CREATE TABLE Answer--------------------------
...
CREATE TABLE [dbo].[Intake](
	...
	constraint Pk_Inake primary key([Intake_ID] )
...
CREATE TABLE [dbo].[Course](
	...
	constraint Fk_Course_Intake foreign key ([Intake_ID]) references Intake([Intake_ID]),
	constraint Fk_Course_Branch foreign key ([Branch_ID]) references Branch(Branch_ID),
	constraint Fk_Instructor_Branch foreign key ([Ins_ID]) references Instructor(Ins_ID),
...
create table Question(
	...
	constraint Fk_Question_Inst foreign key ([Ins_ID]) references Instructor([Ins_ID])
)
go
...
CREATE TABLE [dbo].[Students](
 ...
 constraint Fk_Student_Instructor  foreign key ([Ins_ID]) references Instructor([Ins_ID])
...
CREATE TABLE [dbo].[Instructor_Sets_Exam](
	...
	[Year] [date] NOT NULL,
 CONSTRAINT [PK_Instructor_Sets_Exam] PRIMARY KEY  ([Exam_ID],[Inst_ID])
) ON [PRIMARY]
GO
...
CREATE TABLE [dbo].[Student_Intake](
	[Std_ID] [int] NOT NULL,
	[Intake_ID] [int] NOT NULL,
 CONSTRAINT [PK_Student_Intake] PRIMARY KEY ( [Std_ID],[Intake_ID])
