...
select Students.Std_ID, Std_Name, Std_Age, Exam.[Exam_Degree],[dbo].[Students_take_Exam].Exam_ID,Intake.Intake_ID, Intake_Name,  [Track].[Track_ID], [Track_Name]
...
join
[dbo].[Track]
on [Track].[Track_ID] = [Students].[Track_ID]
join
[dbo].[Exam]
...
on  [Instructor].[Ins_ID] = [Instructor_Choose_Questions].[Instructor_ID]
	join
	[dbo].[Question]
	on [dbo].[Question].[Id] = [dbo].[Instructor_Choose_Questions].[Questions_ID]
join
...
join
		...
		join*/
[dbo].[Branch]
on [Course].[Branch_ID] = [dbo].[Branch].[Branch_ID]
